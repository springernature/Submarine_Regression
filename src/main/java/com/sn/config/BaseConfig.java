...
import com.sn.apt.browser.core.CustomThreadLocalDriver;
import com.sn.apt.browser.core.CustomThreadLocalDriver.DriverType;
...
public class BaseConfig implements IHookable {
	...
	@Parameters({
			...
			"env" })
	public void beforeClass(final String env) {

		configureLocalBrowser(env);
	 ...
	 */
	@SuppressWarnings("squid:S2696")
	private void configureBrowserStack(final String os, final String osVersion, final String browserName,
			final String browserVersion, final String browserstackLocal, final String seleniumVersion,
			final String browserDriver) {
		final ConfigureCapabilities caps = new ConfigureCapabilities(os, osVersion, browserName, browserVersion,
				browserstackLocal, seleniumVersion, browserDriver);
	...
	private void configureLocalBrowser(final String executionEnv) {
		// browserUtil = new
		// CustomChromeDriver(config.getProperty("chromedriverVersion"));

		browserUtil = new CustomThreadLocalDriver(config.getProperty("chromedriverVersion"), DriverType.CHROME);
		...
		baseDriver.get(config.getProperty("stoa_url"));
	...
	public WebDriver getBaseDriver() {
		...
		// return browserUtil.getThreadLocalDriver();
		return baseDriver;
	}
